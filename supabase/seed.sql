...
-- upsert: الترحيل 20260702150000 يُدرج هذه الحاويات برصيد صفر أولًا، فنُحدّث
-- الأرصدة التجريبية هنا محليًا (seed لا يُطبَّق على السحاب فيبقى الرصيد صفرًا هناك).
-- ملاحظة: suppliers_payable لا يُضبَط هنا — يتراكم تلقائيًا عبر محفّز دَين
-- الفاتورة عند إدراج الفواتير التجريبية أدناه (يبقى صفرًا من الترحيل).
...
on conflict (code) do update set balance = excluded.balance, name_ar = excluded.name_ar;
...
-- ── موردون تجريبيون ──
-- الأرصدة صفر: تتراكم تلقائيًا عبر محفّز دَين الفاتورة من الفواتير أدناه
-- (رصيد المورد = مجموع متبقّيات فواتيره).
insert into suppliers (id, name, phone, visit_days, balance, risk_score, red_flag) values
  ('33333333-3333-3333-3333-333333333333', 'مؤسسة النجم للتجارة', '0599111222', '{0,3}', 0, 15, false),
  ('44444444-4444-4444-4444-444444444444', 'شركة المدار للمواد الغذائية', '0599333444', '{1,4}', 0, 5, false),
  ('55555555-5555-5555-5555-555555555555', 'تجارة الكويكب', '0599555666', '{2}', 0, 62, true);
